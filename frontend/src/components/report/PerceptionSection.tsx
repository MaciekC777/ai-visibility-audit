...
export function PerceptionSection({ sentiment, perceptionScore, language }: PerceptionSectionProps) {
  ...
  return (
    <section id="perception" className="scroll-mt-24">
      <h2 className="text-2xl font-display font-bold text-gray-900 mb-6">{t.perceptionAnalysis}</h2>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <Card>
          <CardContent>
            <div className="text-center mb-4">
              ...
              <div className="text-sm text-gray-500 mt-1">{t.perceptionScore}</div>
            </div>
            ...
            <div className="flex justify-between text-xs text-gray-500 mt-2">
              <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-green-400 inline-block" /> {Math.round((pos / total) * 100)}{t.percentPositive}</span>
              <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-gray-300 inline-block" /> {Math.round((neu / total) * 100)}{t.percentNeutral}</span>
              <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-red-400 inline-block" /> {Math.round((neg / total) * 100)}{t.percentNegative}</span>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent>
            {allPraise.length > 0 && (
              <div className="mb-4">
                <h3 className="text-xs font-semibold text-gray-400 uppercase mb-2">{t.whatAIPraises}</h3>
            ...
            {allCriticism.length > 0 && (
              <div>
                <h3 className="text-xs font-semibold text-gray-400 uppercase mb-2">{t.whatAICriticizes}</h3>
                <ul className="space-y-1">
            ...
            {allPraise.length === 0 && allCriticism.length === 0 && (
              <p className="text-sm text-gray-400">{t.noPraiseOrCriticism}</p>
            )}
          </CardContent>
        </Card>
      </div>
