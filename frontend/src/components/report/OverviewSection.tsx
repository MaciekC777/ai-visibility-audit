...
export function OverviewSection({ audit, summary, competitors = [], brandMentions = 0 }: OverviewSectionProps) {
    ...
    <section id="overview" className="scroll-mt-24 space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <ScoreCard
          ...
          value={audit.visibility_score}
          sub="AI discovery rate"
        />
        <ScoreCard
          label="Accuracy Score"
          value={audit.accuracy_score}
          sub="Factual correctness"
        />
        <ScoreCard
          label="Perception Score"
          value={audit.perception_score}
          sub="Sentiment in responses"
        />
        <ScoreCard
          label="Market Rank"
          value={audit.market_rank ? `#${audit.market_rank}` : null}
          sub="vs competitors in AI"
        />
      </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="bg-gray-50 rounded-xl px-5 py-4 text-center">
          ...
          <div className="text-xs text-gray-500 mt-0.5">Total AI mentions</div>
        </div>
        <div className="bg-gray-50 rounded-xl px-5 py-4 text-center">
          <div className="text-2xl font-bold text-gray-800">{modelsCount}</div>
          <div className="text-xs text-gray-500 mt-0.5">AI models tested</div>
        </div>
      </div>
        ...
        <div className="bg-gradient-to-br from-primary-50 to-blue-50 border border-primary-100 rounded-2xl p-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-base font-semibold text-primary-800">
              AI Visibility Summary{audit.brand_name ? ` — ${audit.brand_name}` : ''}
            </h3>
          </div>
