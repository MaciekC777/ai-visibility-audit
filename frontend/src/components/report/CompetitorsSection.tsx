...
export function CompetitorsSection({ competitors, brandName, brandMentions = 0 }: CompetitorsSectionProps) {
  ...
  const maxCount = Math.max(...top.map((c) => c.mentionCount), brandMentions, 1);

  return (
    <section id="competitors" className="scroll-mt-24">
      <h2 className="text-2xl font-display font-bold text-gray-900 mb-6">Competitor Landscape</h2>
            ...
            <div className="space-y-3">
              ...
              {brandName && (
                <div className="flex items-center gap-4">
                  <div className="w-32 text-sm font-semibold text-primary-700 truncate">{brandName}</div>
              ...
              {top.map((c) => (
                <div key={c.name} className="flex items-center gap-4">
                  <div className="w-32 text-sm text-gray-700 truncate">{c.name}</div>
                  <div className="flex-1 bg-gray-100 rounded-full h-3 overflow-hidden">
                    <div
                      className="h-full bg-gray-400 rounded-full"
                      style={{ width: `${(c.mentionCount / maxCount) * 100}%` }}
                    />
                  </div>
                  <div className="text-sm text-gray-600 w-12 text-right">{c.mentionCount}</div>
                </div>
              ))}
            </div>
