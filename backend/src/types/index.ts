...
// ─── v3 Category-specific Extractions ─────────────────────────────────────────

export type PromptCategoryV3 = 'discovery' | 'services' | 'opinions' | 'competitors';
...
export interface DiscoveryExtraction {
  businesses_mentioned: DiscoveryBusiness[];
  brand_found: boolean;
...
export interface ServicesMention {
  service: string;
  description: string;
  accuracy: 'correct' | 'incorrect' | 'unverifiable';
  source_claim: string;
}

export interface PricingMention {
  mentioned: boolean;
  details: string | null;
  accuracy: 'correct' | 'incorrect' | 'unverifiable';
}

export interface HallucinationV3 {
  claim: string;
  reality: string;
...
export interface ServicesExtraction {
  services_mentioned: ServicesMention[];
  pricing_mentioned: PricingMention;
  overall_completeness: 'comprehensive' | 'partial' | 'minimal';
  hallucinations: HallucinationV3[];
  _model?: string;
  _promptId?: string;
}

export interface OpinionsExtraction {
  overall_sentiment: 'positive' | 'neutral' | 'negative' | 'mixed';
  sentiment_score: number;
  pros: string[];
  cons: string[];
  recommendation_strength: 'strong_recommend' | 'soft_recommend' | 'neutral' | 'soft_discourage' | 'strong_discourage';
...
export interface CompetitorMentionV3 {
  name: string;
  position: number;
  context: string;
  sentiment_vs_brand: 'preferred' | 'equal' | 'inferior' | 'neutral';
}

export interface CompetitorsExtraction {
  competitors: CompetitorMentionV3[];
  brand_mentioned: boolean;
  replacement_suggested: boolean;
  total_alternatives: number;
  _model?: string;
  _promptId?: string;
}

export interface AnchorCompetitor {
  name: string;
  mention_count: number;
  contexts: string[];
  sentiments: string[];
...
export interface CategoryAnalysisResult {
  ...
  services: ServicesExtraction[];
  opinions: OpinionsExtraction[];
  competitors: CompetitorsExtraction[];
  anchorList: AnchorCompetitor[];
}
